...
(def ^:const ^double len 0.7) ; length of first rod, second has length (- 1.0 len)
(def ^:const ^double mass1 15) ; mass of the first ball
(def ^:const ^double mass2 4) ; mass of the second ball
...
(def ^:const ^double speed 1.0) ; simulation speed, set to 1.0 to have real time animation
...
(do
  ...
  (defn observe
    ""
    [t ^Struct state]
    (let [[a0 a1] (ss/structure->vector ((.v state) 1))
          posx1 (+ 300 (* l1 (m/sin a0)))
          posy1 (+ 300 (* l1 (m/cos a0)))
