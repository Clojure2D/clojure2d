...
(defn move-particle
  ...
  (let [step-x-shift (* shift-x-step ^double (m/noise time (/ (.x in) shift-x-scale)))
        ^double step-y-shift (m/norm (m/noise (/ (.y in) shift-y-scale) time) 0 1 (- shift-y-step) shift-y-step)
...
(defn example-10
  ...
  (let [canvas (create-canvas width height)
        ...
        particles (vec (repeatedly 500 make-particle))]
