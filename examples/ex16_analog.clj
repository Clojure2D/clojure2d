...
;; filter with dj-eq
(save-pixels (p/filter-channels p/normalize (p/filter-channels (make-effects-filter [effect1]) img)) "results/ex16/djeq.jpg")

;; filter with 3 lowpass
(save-pixels (p/filter-channels p/normalize (p/filter-channels (make-effects-filter [effect2 effect2 effect2]) img)) "results/ex16/lowpass3.jpg")
